...
The trick is that it emits the **same** forgetting-curve half-life a trained
...
reading the stored half-life and never learn which produced it. Once the pair has
...
half-life is solved so the curve crosses the reference threshold ``θ`` exactly at
``I``::

    θ = 2**(−I/H)   ⇒   H = I / log2(1/θ)

which is :func:`~src.model.curve.invert_curve` run backwards. The stored half-life
therefore stays threshold-independent — the user's own recall threshold is applied
...
class HeuristicPredictor:
    ...
    Exposes the same ``config`` / ``half_life`` / ``recall_probability`` /
    ...
    def half_life(
        ...
        """Return the curve half-life, in seconds, implied by this history.

        Args:
            reps: Repetition history for one (word, direction), oldest first.
                Must be non-empty.
            direction: Accepted for signature-compatibility with
                :meth:`Predictor.half_life` and ignored — the history is already
        ...
        # Solve H so the curve crosses the reference threshold exactly at `interval`.
        return interval / math.log2(1.0 / h.reference_threshold)
    ...
    def recall_probability(
        ...
        return recall_at(self.half_life(reps, direction), delta_seconds)
    ...
    def post_rep_half_lives(
        ...
        The model-free counterpart of :meth:`Predictor.post_rep_half_lives`: it
        ...
        Note that the heuristic's half-life follows the ease ladder and the gap
        ...
        Args:
            ...
            chunk_size: Accepted for signature-compatibility with
                :meth:`Predictor.post_rep_half_lives` and ignored — there is no
        ...
        for reps, direction in histories:
            ...
            out.append(lives)  # type: ignore[arg-type]
        return out
